...
# Program: ur_jalex1.py
# Author: "Joel Alex"
# The python code in this file ur_jalex1.py is original work written by
...
#Creating this function for monthly usage

def cal_monthly_usage(subject, login_recs):
  
    message = ""
    message += "Monthly usage report for " + str(subject)
    message += "\a"
    for length in range (1,len(message)):
        message += "="
    message += "\a"
    ...
    for item in range(0, len(login_recs)):
        ...
        messageofdate = time.strftime("%Y %m", time.localtime(scnds1))
        monthly_usage = int(monthly_usage)

        # Checks if the date already exists then it replaces the seconds
        if messageofdate in message:
            if t in message:
                message = message.replace(t, str(days))
                tmp = []
    ...
    if args.user:
        if args.type == 'daily':
            ...
            if args.verbose:
                ...
                print("reading login/logout record files " + str(args.F))
            fileused = read_login_rec(args.user, args.F)
            print(cal_monthly_usage(args.user, fileused))
    ...
    if args.rhost:
        if args.type == 'daily':
            ...
            if args.verbose:
                ...
                print("reading login/logout record files " + str(args.F))
            fileused = read_login_rec(args.rhost, args.F)
            print(cal_monthly_usage(args.rhost, fileused))  
